-- ============================================================================
-- APPROVE REQUESTED ZSAES USERS AS SUPERVISORS
-- ============================================================================
-- Run this whole script in the Supabase SQL Editor with a privileged role.
--
-- Login password for all users below:
--   ZSAES_123456
--
...
--   - public.profiles rows with role = supervisor and status = approved
-- ============================================================================
...
create temporary table requested_supervisors (
  email text primary key,
  first_name text not null,p
  last_name text not null,
  clear_password text not null
) on commit preserve rows;

insert into requested_supervisors (email, first_name, last_name, clear_password)
values
  ('nshayanewako@zsaes.org.zw', 'Nshaya', 'Newako', 'ZSAES_123456'),
  ('emupanehari@zsaes.org.zw', 'E', 'Mupanehari', 'ZSAES_123456'),
  ('labanalysts@zsaes.org.zw', 'Lab', 'Analysts', 'ZSAES_123456')
...
begin
  ...
  execute format(
    'insert into auth.identities (%s)
     select %s
     from auth.users u
     join requested_supervisors s on lower(u.email) = s.email
     where not exists (
       select 1
